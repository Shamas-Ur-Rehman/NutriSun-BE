...
func AssignPermissionsToRole(c *gin.Context, masterDB *gorm.DB) {
	...
	if err := masterDB.Where("business_id = ? AND branch_id = ?", businessID, branchID).First(&user, userID).Error; err != nil {
	...
	if err := masterDB.Where("business_id = ? AND branch_id = ?", businessID, branchID).First(&role, req.RoleID).Error; err != nil {
		c.JSON(http.StatusNotFound, utils.ErrorResponse{
			Status:  http.StatusNotFound,
	...
	masterDB.Where("business_id = ? AND branch_id = ? AND role_id = ?", businessID, branchID, req.RoleID).Find(&existingPermissions)
	...
	if len(permissionsToCreate) > 0 {
		if err := masterDB.CreateInBatches(permissionsToCreate, 100).Error; err != nil {
			c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
				Status:  http.StatusInternalServerError,
	...
	if len(permissionsToDelete) > 0 {
		masterDB.Where("id IN ?", permissionsToDelete).Delete(&models.Permission{})
	}

	go middleware.RefreshPermissionCache(masterDB)
...
func RemovePermissionsFromRole(c *gin.Context, masterDB *gorm.DB) {
	userIDValue, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse{
			Status:  http.StatusUnauthorized,
	...
	if err := masterDB.Where("business_id = ? AND branch_id = ? AND id IN ? AND role_id = ?", businessID, branchID, req.PermissionIDs, req.RoleID).Delete(&models.Permission{}).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
	...
	masterDB.Where("business_id = ? AND branch_id = ? AND role_id = ?", businessID, branchID, req.RoleID).Find(&permissions)
	...
	var modules []models.Module
	if len(moduleIDs) > 0 {
		masterDB.Where("business_id = ? AND branch_id = ? AND id IN ?", businessID, branchID, moduleIDs).Find(&modules)
	}
	moduleMap := make(map[uint]models.Module, len(modules))
	for _, module := range modules {
		moduleMap[module.ID] = module
...
func GetPermissionsByRoleID(c *gin.Context, masterDB *gorm.DB) {
	roleID := c.Param("id")
	authContext, ok := utils.GetCurrentUserContext(c)
	if !ok {
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse{
