...
func Login(c *gin.Context, masterDB *gorm.DB) {
	...
	if err := masterDB.Where("email = ? AND is_active = ?", req.Email, true).First(&user).Error; err != nil {
	...
	if err := masterDB.First(&business, user.BusinessId).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
	...
	if err := masterDB.First(&role, user.RoleID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
	...
	token, err := utils.GenerateJWT(user.ID, user.RoleID, user.Email, business.DB, user.Service)
	if err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
...
func GetUser(c *gin.Context, masterDB *gorm.DB) {
...
func Logout(c *gin.Context, masterDB *gorm.DB) {
	userIDValue, exists := c.Get("user_id")
	if exists {
		userID, ok := userIDValue.(uint)
		if ok {
