...
func CreateUser(c *gin.Context, masterDB *gorm.DB) {
	var req struct {
	...
	if err := masterDB.Where("email = ?", req.Email).First(&existingUser).Error; err == nil {
	...
	if employeeId == "" {
		generatedId, err := utils.GenerateNextEmployeeID(masterDB)
		if err != nil {
			c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
				Status:  http.StatusInternalServerError,
		...
		if err := masterDB.Where("employee_id = ?", employeeId).First(&existingEmp).Error; err == nil {
			c.JSON(http.StatusConflict, utils.ErrorResponse{
				Status:  http.StatusConflict,
	...
	if err := masterDB.Select("id, name").First(&role, req.RoleID).Error; err != nil {
		c.JSON(http.StatusBadRequest, utils.ErrorResponse{
			Status:  http.StatusBadRequest,
	...
	masterDB.Select("id, name_en, name_ar").First(&business, user.BusinessId)
...
func GetAllUsers(c *gin.Context, masterDB *gorm.DB) {
	...
	query := masterDB.Model(&models.User{}).Where("business_id = ? AND branch_id = ?", authContext.BusinessID, authContext.BranchID)
	...
	if err := query.Offset(offset).Limit(utils.StringToInt(limit)).Find(&users).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
	...
	if len(roleIDs) > 0 {
		masterDB.Select("id, name").Where("id IN ?", roleIDs).Find(&roles)
	}

	if len(businessIDs) > 0 {
		masterDB.Select("id, name_en, name_ar").Where("id IN ?", businessIDs).Find(&businesses)
	}
...
func GetUserByID(c *gin.Context, masterDB *gorm.DB) {
	userID := c.Param("id")
	authContext, ok := utils.GetCurrentUserContext(c)
	if !ok {
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse{
	...
	if err := masterDB.Where("business_id = ? AND branch_id = ?", authContext.BusinessID, authContext.BranchID).First(&user, userID).Error; err != nil {
	...
	var role models.Role
	var business models.Business
	masterDB.Select("id, name").First(&role, user.RoleID)
	masterDB.Select("id, name_en, name_ar").First(&business, user.BusinessId)
...
func UpdateUser(c *gin.Context, masterDB *gorm.DB) {
	userID := c.Param("id")
	authContext, ok := utils.GetCurrentUserContext(c)
	if !ok {
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse{
	...
	if req.Email != "" && req.Email != user.Email {
		var existingUser models.User
		if err := masterDB.Where("email = ? AND id != ?", req.Email, userID).First(&existingUser).Error; err == nil {
			c.JSON(http.StatusConflict, utils.ErrorResponse{
				Status:  http.StatusConflict,
				Message: "Email already exists",
			})
	...
	if req.RoleID != 0 {
		var role models.Role
		if err := masterDB.Select("id").First(&role, req.RoleID).Error; err != nil {
			c.JSON(http.StatusBadRequest, utils.ErrorResponse{
				Status:  http.StatusBadRequest,
				Message: "Role not found",
			})
...
func DeleteUser(c *gin.Context, masterDB *gorm.DB) {
	userID := c.Param("id")
	authContext, ok := utils.GetCurrentUserContext(c)
	if !ok {
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse{
	...
	if err := masterDB.Delete(&user).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
...
func ChangePassword(c *gin.Context, masterDB *gorm.DB) {
	...
	var user models.User
	if err := masterDB.Where("business_id = ? AND branch_id = ?", authContext.BusinessID, authContext.BranchID).First(&user, req.UserID).Error; err != nil {
		c.JSON(http.StatusNotFound, utils.ErrorResponse{
			Status:  http.StatusNotFound,
			Message: "User not found",
		})
...
func GetMyPermissions(c *gin.Context, masterDB *gorm.DB) {
	...
	var role models.Role
	if err := masterDB.Select("id, name").First(&role, user.RoleID).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
	...
	if err := masterDB.Where("role_id = ?", user.RoleID).Find(&permissions).Error; err != nil {
		c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
			Status:  http.StatusInternalServerError,
	...
	if len(moduleIDs) > 0 {
		if err := masterDB.Where("id IN ?", moduleIDs).Find(&modules).Error; err != nil {
			c.JSON(http.StatusInternalServerError, utils.ErrorResponse{
				Status:  http.StatusInternalServerError,
...
func CheckPermission(c *gin.Context, masterDB *gorm.DB) {
	userIDValue, exists := c.Get("user_id")
	if !exists {
		c.JSON(http.StatusUnauthorized, utils.ErrorResponse{
			Status:  http.StatusUnauthorized,
	...
	var user models.User
	if err := masterDB.Select("id, role_id").First(&user, userID).Error; err != nil {
		c.JSON(http.StatusNotFound, utils.ErrorResponse{
			Status:  http.StatusNotFound,
			Message: "User not found",
		})
	...
	if err := masterDB.Select("id").Where("name = ?", req.ModuleName).First(&module).Error; err != nil {
		c.JSON(http.StatusOK, utils.SuccessResponse{
			Status:  http.StatusOK,
	...
	var count int64
	masterDB.Model(&models.Permission{}).
